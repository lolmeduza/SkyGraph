export function getPanelScript(): string {
  const lines = [
    ...
    '    let chats = [{ id: LLM_ONLY_CHAT_ID, name: \'Чат с LLM\', messages: [] }];',
    ...
    '      return out;',
    '    }',
    '    var currentDiffFiles = [];',
    '    var currentDiffIndex = 0;',
    '    function openDiffAt(idx) {',
    '      if (!currentDiffFiles.length) return;',
    '      currentDiffIndex = Math.max(0, Math.min(idx, currentDiffFiles.length - 1));',
    '      var f = currentDiffFiles[currentDiffIndex];',
    '      vscode.postMessage({ type: \'openFileDiff\', path: f.path, originalContent: f.originalContent, proposedContent: f.proposedContent });',
    '      updateDiffNav();',
    '    }',
    '    function updateDiffNav() {',
    '      var nav = document.getElementById(\'diffNav\');',
    '      if (!nav) return;',
    '      var prevBtn = nav.querySelector(\'.diff-nav-prev\');',
    '      var nextBtn = nav.querySelector(\'.diff-nav-next\');',
    '      var counter = nav.querySelector(\'.diff-nav-counter\');',
    '      if (prevBtn) prevBtn.disabled = currentDiffIndex <= 0;',
    '      if (nextBtn) nextBtn.disabled = currentDiffIndex >= currentDiffFiles.length - 1;',
    '      if (counter) counter.textContent = (currentDiffIndex + 1) + \' / \' + currentDiffFiles.length;',
    '      currentDiffFiles.forEach(function (f, i) {',
    '        var row = document.getElementById(\'diff-row-\' + i);',
    '        if (row) row.classList.toggle(\'diff-row-active\', i === currentDiffIndex);',
    '      });',
    '    }',
    ...
    '      if (summaryEl) {',
    '        var parts = [];',
    '        if (created.length) parts.push(\'Созданы: \' + created.join(\', \'));',
    '        if (modified.length) parts.push(\'Изменены: \' + modified.join(\', \'));',
    '        summaryEl.textContent = parts.join(\'. \') || \'\';',
    '      }',
    '      files.forEach(function (f, i) {',
    '        var row = document.createElement(\'div\');',
    '        row.className = \'diff-file-row\' + (i === 0 ? \' diff-row-active\' : \'\');',
    '        row.id = \'diff-row-\' + i;',
    '        var isNew = !(f.originalContent && f.originalContent.trim().length > 0);',
    '        var badge = document.createElement(\'span\');',
    '        badge.className = \'diff-badge \' + (isNew ? \'diff-badge-new\' : \'diff-badge-modified\');',
    '        badge.textContent = isNew ? \'+ новый\' : \'~ изменён\';',
    '        var pathSpan = document.createElement(\'span\');',
    '        pathSpan.className = \'diff-file-path\';',
    '        pathSpan.title = f.path;',
    '        var parts = f.path.split(\'/\');',
    '        pathSpan.textContent = parts.length > 2 ? \'…/\' + parts.slice(-2).join(\'/\') : f.path;',
    '        var showBtn = document.createElement(\'button\');',
    '        showBtn.className = \'diff-open-file-btn\';',
    '        showBtn.textContent = \'Открыть diff\';',
    '        (function (idx) { showBtn.onclick = function () { openDiffAt(idx); }; })(i);',
    '        row.appendChild(badge);',
    '        row.appendChild(pathSpan);',
    '        row.appendChild(showBtn);',
    '        diffViewFiles.appendChild(row);',
    '      });',
    '      if (files.length > 1) {',
    ...
    '        prevBtn.className = \'diff-nav-prev\';',
    '        prevBtn.textContent = \'← Пред\';',
    '        prevBtn.disabled = true;',
    '        prevBtn.onclick = function () { openDiffAt(currentDiffIndex - 1); };',
    '        var counter = document.createElement(\'span\');',
    '        counter.className = \'diff-nav-counter\';',
    '        counter.textContent = \'1 / \' + files.length;',
    '        var nextBtn = document.createElement(\'button\');',
    '        nextBtn.className = \'diff-nav-next\';',
    '        nextBtn.textContent = \'След →\';',
    '        nextBtn.disabled = files.length <= 1;',
    '        nextBtn.onclick = function () { openDiffAt(currentDiffIndex + 1); };',
    '        nav.appendChild(prevBtn);',
    '        nav.appendChild(counter);',
    '        nav.appendChild(nextBtn);',
    '        diffViewFiles.appendChild(nav);',
    '      }',
    ...
    '      t = t.replace(/^[-*]\\s+/gm, \'<span class="msg-bullet">•</span> \');',
    '      t = t.replace(/\\n/g, \'<br>\');',
    '      return t;',
    '    }',
    ...
    '      codeHeader.textContent = lang || \'Код\';',
    ...
    '        navigator.clipboard.writeText(code).then(function () { copyBtn.textContent = \'Скопировано!\'; setTimeout(function () { copyBtn.textContent = \'Копировать\'; }, 2000); }).catch(function () { copyBtn.textContent = \'Ошибка\'; setTimeout(function () { copyBtn.textContent = \'Копировать\'; }, 2000); });',
    '      };',
    ...
    '                thinkSum.textContent = \'Рассуждение\';',
    ...
    '        base = \'Контекст: \' + data.used.toLocaleString() + \' / \' + (data.limit || contextLimit).toLocaleString() + \' (осталось \' + left.toLocaleString() + \')\';',
    '      } else {',
    '        base = \'Контекст: — / \' + contextLimit.toLocaleString();',
    '      }',
    '      if (lastProjectContextChars > 0) base += \' | Контекст проекта (в промпт): \' + lastProjectContextChars.toLocaleString() + \' симв.\';',
    ...
    '        openBtn.textContent = openIds.indexOf(c.id) >= 0 ? \'Открыт\' : \'Открыть\';',
    ...
    '          var newName = window.prompt(\'Название чата\', c.name);',
    ...
    '      if (!text || isRequestActive) return;',
    '      const chat = chats.find(function (c) { return c.id === activeId; });',
    ...
    '    if (stopBtn) stopBtn.addEventListener(\'click\', function () { vscode.postMessage({ type: \'stopRequest\', chatId: activeId }); setRequestActive(false); });',
    ...
    '          chats = msg.chats;',
    '          if (!chats.some(function (c) { return c.id === LLM_ONLY_CHAT_ID; })) chats.unshift({ id: LLM_ONLY_CHAT_ID, name: \'Чат с LLM\', messages: [] });',
    '          var projectChats = chats.filter(function (c) { return c.id !== LLM_ONLY_CHAT_ID; });',
    '          for (var i = 0; i < projectChats.length; i++) projectChats[i].name = \'Чат \' + (i + 1);',
    ...
    '            lastMsg.text = (msg.tool === \'think\' ? \'Рассуждаю...\' : \'Использую: \' + msg.tool + \'...\');',
    ...
    '        hideDiffView();',
    '        if (contextFooter) contextFooter.textContent = \'Правки применены.\';',
    ...
    '        if (contextFooter) contextFooter.textContent = \'Ошибка: \' + msg.message;',
    ...
    '      }',
    '      if (msg.type === \'requestStarted\' && msg.chatId === activeId) { setRequestActive(true); }',
    '      if (msg.type === \'requestFinished\') {',
    '        if (msg.chatId === activeId) setRequestActive(false);',
    '        // Убираем typing placeholder если запрос был остановлен (ответа не будет)',
    '        var stoppedChat = chats.find(function (c) { return c.id === msg.chatId; });',
    '        if (stoppedChat) {',
    '          var lastMsg = stoppedChat.messages[stoppedChat.messages.length - 1];',
    '          if (lastMsg && lastMsg.typing) { stoppedChat.messages.pop(); if (msg.chatId === activeId) renderMessages(); }',
    '        }',
    '      }',
    '    });',
    '',
    ...
    '      var actionLabels = { modify: \'изменить\', create: \'создать\', delete: \'удалить\', rename: \'переименовать\' };',
    '      var stepsHtml = plan.steps.map(function(s, i) {',
    '        var cls = \'plan-step plan-step--\' + s.action;',
    '        var rename = s.newFile ? \' → \' + s.newFile : \'\';',
    ...
    '      }).join(\'\');',
    '      var notesHtml = plan.notes ? \'<div class="plan-notes">📌 \' + plan.notes + \'</div>\' : \'\';',
    '      card.innerHTML = \'<div class="plan-header"><span class="plan-title">📋 \' + plan.title + \'</span></div><div class="plan-steps">\' + stepsHtml + \'</div>\' + notesHtml + \'<div class="plan-actions"><button class="plan-btn plan-btn--confirm" id="plan-confirm-\' + chatId + \'">▶ Выполнить план</button><button class="plan-btn plan-btn--reject" id="plan-reject-\' + chatId + \'">✕ Отменить</button></div>\';',
