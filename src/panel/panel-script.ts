export function getPanelScript(): string {
  const lines = [
    ...
    '        badge.textContent = isNew ? \'Новый файл\' : \'Изменён\';',
    ...
    '        openBtn.title = \'Открыть файл в редакторе\';',
    ...
    '      vscode.postMessage({ type: \'send\', text: text, chatId: activeId, messages: messages, noProjectContext: activeId === LLM_ONLY_CHAT_ID });',
    '      inputEl.value = \'\';',
    '    }',
    '',
    ...
    '        var toast = document.getElementById(\'fileInsertToast\');',
    '        if (toast) { toast.textContent = \'Добавлен файл: \' + msg.path; toast.style.display = \'block\'; window.clearTimeout(window._fileInsertToastTimer); window._fileInsertToastTimer = window.setTimeout(function () { toast.style.display = \'none\'; }, 2500); }',
    '        inputEl.value = \'Файл: \' + msg.path + \' \';',
    '        inputEl.focus();',
    '      }',
    ...
    '      if (overlay) {',
    '        setTimeout(function () { overlay.classList.add(\'starfield-out\'); }, 2200);',
